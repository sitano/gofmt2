...
func (p *printer) declList(list []ast.Decl) {
	...
	for i, d := range list {
		...
		// Put empty line between multiline funcs decls
		if tok == token.FUNC && i > 0 && min < 2 {
			pf := list[i-1]
			prevOneLiner := p.lineFor(pf.Pos()) == p.lineFor(pf.End())
			curOneLiner := p.lineFor(d.Pos()) == p.lineFor(d.End())
			if !prevOneLiner || !curOneLiner {
				if p.lineFor(pf.End())+1 == p.lineFor(d.Pos()) {
					p.linebreak(p.lineFor(d.Pos()), 1, ignore, false)
				}
			}
		}
		p.decl(d)
